...
// PROMPT: Given 1000 digits of PI as strings, return an array of the digits as numbers
// Code plan
// need to loop through each item (use .map) and parseInt each index
// expected outcome is arrNums(array of numbers)

let arrNums = []

const stringsToNumbs = (arr) => {
  arrNums = arr.map((number) => {
...
// PROMPT: With the same numbers, find the sum of the even values
// Code Plan
// loop through each item and mesure if it can be divided by two
// if is has any left over fromt being divided by 2 then it is not even (use %)
// create a bucket to hold the sum
// add all even numbers in bucket
// return the sum
const sumEvens = (arr) => {
  let sum = 0
...
console.log(sumEvens(arrNums))

// PROMPT: Find the index of the first value when added to it's index = 512 (#ATX!!)
// Code Plan
// Loop through the array and find the index + value that equals 512
// create a bucket to hold index and value
...
// PROMPT: Using a higher order function, create an array of the unique 'weather_state_name' values of the weather array. Your function should return the following array ['Light Cloud', 'Heavy Cloud', 'Showers']
// Code Plan
// loop through each item to find anything that does not include the weather state name key
// create a bucket to push all of the entries for the weather state name key
// return the bucket with the weather state name key value
const weatherStates = (arr) => {
  const wsn = []
  for (let i = 0; i < arr.length; i++) {
    arr.forEach(() => {
      if (!wsn.includes(arr[i].weather_state_name)) {
        wsn.push(arr[i].weather_state_name)
      }
    })
  }
  return wsn
...
// PROMPT: Find the id of the object in weather that has a min_temp of 15.915
// Code Plan
// create a loop that can find the item that has a min temp = to 15.915
// then return the id number number for that object
const idealTemp = (arr) => {
  for (let i = 0; i < arr.length; i++) {
